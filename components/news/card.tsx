...
export default function NewsCard ({ url, image, title, published, description } : { url: string, image: string, title: string, published: string, description: string }) {
    return (
        <>
            <motion.div initial={{ opacity: 0, x: -20 }} whileInView={{ opacity: 1, x: 0 }} whileHover={{ scale: 1.03 }} whileTap={{ scale: 0.9 }} transition={{ type: "spring",  duration: 0.5 }} >
                <a href={url}>
                    <Card placeholder="" className="h-96 m-6">
                        <CardHeader placeholder="" className="absolute inset-0 m-0 h-full w-full bg-cover bg-center rounded-lg" style={{ backgroundImage : `url(${image})` }}>
